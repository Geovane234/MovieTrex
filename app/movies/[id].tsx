...
const MovieInfo = ({ label, value }: MovieInfoProps) => (
    <View style={tw`flex-col items-start justify-center mt-5`}>
        <Text style={tw`text-light-200 font-normal text-sm`}>{label}</Text>
        <Text style={tw`text-light-100 font-bold text-sm mt-2`}>
            {value || "N/A"}
        </Text>
    </View>
);
        ...
        <View style={tw`bg-primary flex-1`}>
            <ScrollView contentContainerStyle={{ paddingBottom: 80 }}>
                <View>
                    ...
                    <TouchableOpacity style={tw`absolute bottom-5 right-5 rounded-full size-14 bg-white flex items-center justify-center`}>
                        <Image
                            source={icons.play}
                ...
                <View style={tw`flex-col items-start justify-center mt-5 px-5`}>
                    ...
                    <View style={tw`flex-row items-center gap-x-1 mt-2`}>
                        <Text style={tw`text-light-200 text-sm`}>
                            {movie?.release_date?.split("-")[0]} •
                        </Text>
                        <Text style={tw`text-light-200 text-sm`}>{movie?.runtime}m</Text>
                    </View>

                    <View style={tw`flex-row items-center bg-dark-100 px-2 py-1 rounded-md gap-x-1 mt-2`}>
            ...
            >
                <Image
                    source={icons.arrow}
                    style={tw`size-5 mr-1 mt-0.5 rotate-180`}
                    tintColor="#fff"
                />
